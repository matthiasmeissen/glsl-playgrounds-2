...
void main(void) {
    ...
    // Base Image
    float d = 1.0 - length(vec2(uv.x, uv.y - 0.2) * 1.2);
    d = smoothstep(0.2, 0.8, d);

    color = pal(d, vec3(0.5,0.5,0.5),vec3(0.5,0.5,0.5),vec3(1.0,0.6,0.4),vec3(0.0,0.15,0.20));
    ...
    float s = fract(uv.y * 4.0 - u_time) + 0.01;
    float n = noise(vec3(uv.x * 8.0, uv.y + s * 0.4, u_time * 0.4));
    vec2 d = sin(uv / s * 0.2) + n;

    d = (d + 1.0) / 2.0;

    vec2 displacement_map = vec2(d);
    vec2 offset = (displacement_map - mid) * scale;
    vec2 disp = st + offset;
    
    vec3 baseTex = texture2D(u_doubleBuffer0, disp).rgb;
