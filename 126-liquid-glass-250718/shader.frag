...
float sceneSDF(vec3 p) {
    ...
    float s1 = fSphere(vec3(p.x, p.y - sin(u_time) * 1.2, p.z), d);
    dist = fOpUnionRound(dist, s1, 0.4);

    float s2 = fSphere(vec3(p.x, p.y + sin(u_time * 1.4) * 1.2, p.z), 1.5);
    dist = fOpUnionRound(dist, s2, 0.4);
...
void main(void) {
    ...
    vec2 p = abs(st);
    float tile = fract(pow(p.x, 1.2) * st.y * 8.0 - u_time);
    float notch_shape = pow(abs(tile - 0.5) * 2.0, 1.2);
    p.x += 0.4 * notch_shape;

    vec2 disp = st + vec2(p.x, 0.0) * 0.1;
