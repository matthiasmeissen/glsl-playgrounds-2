...
float map(vec3 p) {
    ...
    torus = sdTorus(q * rotationX(u_time) * rotationZ(sin(p.x)), vec2(1.0, 0.1));
    d = opSmoothUnion(d, torus, 0.4);

    torus = sdTorus(q * rotationY(-u_time) * rotationZ(sin(p.y)), vec2(0.8, 0.2));
    d = opSmoothUnion(d, torus, 0.4);

    torus = sdTorus(vec3(q.x + 1.5, q.y, q.z), vec2(1.4, 0.1));
    d = opSmoothUnion(d, torus, 0.4);
...
Material getMaterial(vec3 p) {
    ...
    mat.ior = 1.8; // 1.1 Air/Water, 2.4 Diamond
    mat.absorption = vec3(0.1, 0.1, 0.1); 
...
vec3 getEnvironment(vec3 rayDir, float roughness) {
    ...
    vec3 bg = vec3(1.0);
    bg *= smoothstep(rayDir.z, rayDir.y, fract(length(uv + rayDir.y) + rayDir.z + u_time * 0.4));
...
void main() {
    ...
    vec3 lookAt = vec3(0.0, mix(-2.0, 2.0, uParam1), 0.0);
    ...
    col = palette(col.r, vec3(0.5), vec3(0.5), vec3(2.0, 1.0, 1.0), vec3(0.2, 0.2, 0.25));
